...
func drawQuadro(screen *ebiten.Image, tree *QuadTree) {
	...
	for _, node := range tree.nodes {
		DrawLineBresenham(screen,
			int(node.xmin*float32(size)),
			int(node.ymin*float32(size)),
			int(node.xmin*float32(size)),
			int(node.ymax*float32(size)),
			color.Gray{128},
		)
		DrawLineBresenham(screen,
			int(node.xmin*float32(size)),
			int(node.ymin*float32(size)),
			int(node.xmax*float32(size)),
			int(node.ymin*float32(size)),
			color.Gray{128},
		)
		DrawLineBresenham(screen,
	...
	for _, point := range points {
		drawCircle(screen,
			int(point.x*float32(size)),
			int(point.y*float32(size)),
			5,
	...
	tree.leafGraph.Each(func(u QuadTreeNodeID, neighbour *treemap.Map[QuadTreeNodeID, float32]) {
		neighbour.Each(func(v QuadTreeNodeID, _ float32) {
			...
			DrawThickLine(screen,
				int(uNode.x()*float32(size)),
				int(uNode.y()*float32(size)),
				int(vNode.x()*float32(size)),
				int(vNode.y()*float32(size)),
				3,
